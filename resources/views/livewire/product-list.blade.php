<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    {{-- @if(count($batch_list) > 0) --}}
        <div style="width: 60%;display:none" id="batches_in_progress" class="mb-4 grid grid-cols-1 rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <h2 class="text-title-md2 font-bold text-black dark:text-white mb-1">Batches in progress</h2>
            <div class="max-w-full overflow-x-auto mb-4" id="progressBarprogressBar">
            </div>               
        </div>
    {{-- @endif --}}
    @if (session()->has('all_batch_completed'))
                    ...
                    <div style="background-color: rgba(0, 0, 0, .5)" class="mx-auto absolute top-0 left-0 w-full h-full flex items-center shadow-lg overflow-y-auto" x-show="isOpen">
                        <div class="container mx-auto rounded p-4 mt-2 overflow-y-auto w-full xl:w-1/2">
                            <div class="bg-white rounded px-8 py-8">
                                <!-- Alerts Item -->
                                @if (session()->has('one_batch_completed'))
                                    <div class="grid grid-cols-1 gap-4 mb-4">
                                        <div class="col-span-1 flex border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] dark:bg-[#1B1B24] px-7 py-3 shadow-md dark:bg-opacity-30">
                                            <div class="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-[#34D399]">
                                            <svg width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
    ...
    document.addEventListener('jsFunctionTriggered', event => {
        ...
        var batch_id = event.detail.batch_id;
        currentProgress = 0;
        renderComponents = 0;
        setInterval(function () {
            if(currentProgress != 100){
            ...
            if(currentProgress == 100 && renderComponents == 1){
                ...
                //get updated data
                window.livewire.emit('render','one_batch_completed'); 
            }
        }, 1000);
    });

    //on page load
    var renderComponent1 = 0;
    function pendingJobBatches(){
        ...
        let url = window.location.origin+"/get-pending-job-batches";
            setInterval(function () {
                if(pending_job_batches > 0){
                    const batchesInProgressDiv = document.getElementById("batches_in_progress");
                    batchesInProgressDiv.style.display = "block";
                    fetch(url)
                    .then(response => response.json())
                        .then(data => {
                            let batches = data.batches_list;
                            progressBarprogressBar.innerText = "";
                            pending_job_batches = data.batches_list.length;
                            if(pending_job_batches == 0){
                                renderComponent1 = 1;
                            }
                            batches.forEach(function(batch) {
                                ...
                                progressBar.innerHTML = `<h2>Product status: (${batch.total_records}/${batch.total_added})</h2><div class="rounded-full dark-bg-gray mt-2"> <div id="${batch.finished_at}" class="bg-primary bg-blue-600 text-xs font-medium text-white text-center p-0.5 leading-none rounded-full" style="width:${progressPercentage.toFixed(2)}%">${progressPercentage.toFixed(2)}%</div></div>`;
                ...
                //get the inserted record when all pending job batches completed
                if(pending_job_batches == 0 && renderComponent1 == 1){
                    renderComponent1 = 0;
                    window.livewire.emit('render','all_batch_completed'); 
                }
            }, 1000);    
    }
    //when upload the file then hit from livewire compoent
    document.addEventListener('showProgressBar', event => {
        let inputField = document.getElementById('file');
        inputField.value = null;
        pending_job_batches = 1;
        pendingJobBatches();
    });
    //on plage load run this if pending job greater then 0 
    if(pending_job_batches > 0){
        pendingJobBatches();
    }
